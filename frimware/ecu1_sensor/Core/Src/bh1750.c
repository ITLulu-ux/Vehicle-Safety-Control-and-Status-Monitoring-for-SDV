...
uint8_t 			bh1750Mtreg;	// Current MT register value
...
{
	if(BH1750_OK == BH1750_SetMode(bh1750Mode))
	{
		return BH1750_OK;
	}
	return BH1750_ERROR;
...
BH1750_STATUS BH1750_ReadLight(float *Result)
{
	...
	if(HAL_OK == HAL_I2C_Master_Receive(bh1750_i2c, BH1750_ADDRESS, tmp, 2, 10))
	{
		...
		if(bh1750Mtreg != BH1750_DEFAULT_MTREG)
		{
			result *= (float)((uint8_t)BH1750_DEFAULT_MTREG/(float)bh1750Mtreg);
		}

		if(bh1750Mode == ONETIME_HIGH_RES_MODE_2 || bh1750Mode == CONTINUOUS_HIGH_RES_MODE_2)
		{
			result /= 2.0;
		}
